...
const MovieList = () => {
    ...
    const getAlls = async () => {
        let data = await getMovieByGenre("", "single")
        return data
    }
    ...
    const getMovieByGenres = async (genre) => {
        let data = await getMovieByGenre(genre, "single")
        return data
    }
                    ...
                    <ul className="cate-list">
                        <li className="cate-item"><button onClick={() => changeMovieByGenre("")}>Tất cả</button></li>
                        {
                            genreList && genreList.length > 0 && genreList.map((item, index) => {
                                return (
                                    <li key={index * 273} className="cate-item"><button onClick={() => changeMovieByGenre(item.genre_vie)}>Phim {item.genre_vie}</button></li>
                                )
                            })
                        }
                    ...
                    <div className="row ipad-width2">
                        <div className="col-md-12 col-sm-12 col-xs-12 content-col-12">
                            <div class="title-hd"><h2>{
                                genreName === "" ? `Tất cả` : `Phim ${genreName}`
                            }</h2>
...
<div className="number-views">
                                                    ...
                                                    <p>{item.view}</p>
                                                            ...
                                                            <Link to={`/detail/${item.movie_id}/${item.slug}`}>Read more <i className="ion-android-arrow-dropright"></i></Link>
                                                        </div> */}
